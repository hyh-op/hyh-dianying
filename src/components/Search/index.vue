				...
				<div class="search_result">
					...
					<ul>
						...
						<li v-for="data in movieList" :key="data.id">
							<div class="img"><img :src="data.img | setImg(data.img)"></div>
							<div class="info">
								<p><span>{{data.nm}}</span><span v-if="data.sc">{{data.sc}}</span>
								<span v-else style="color:#999;">暂无评分</span></p>
								<p>{{data.enm}}</p>
								<p>{{data.cat}}</p>
								<p>{{data.rt}}</p>
							</div>
...
export default {
	...
	watch : {
		message(newVal){
			...
			this.axios.get(`/apollo/ajax/search?kw=${newVal}&cityId=151&stype=-1`, {
